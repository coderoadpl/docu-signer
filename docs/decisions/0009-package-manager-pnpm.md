...
3. **Dependency lifecycle scripts stay off, and the exceptions are an explicit
   ...
   **`onlyBuiltDependencies`**. That list is a security control, not
   ...
   - today's tree needs it only for native/binary resolution (`esbuild` and
     ...
     whatever a green `check` / `smoke` / `e2e` proves
     necessary — no more.
...
7. **Every install path changes together**: the `ci` and `visual-baselines`
   workflows install with a frozen lockfile and cache the pnpm store instead of
   the npm cache; Vercel builds through the `packageManager` pin. Any GitHub
   Action added to set pnpm up is pinned by full commit SHA like every other
   `uses:` (ADR-0004 §5).
...
- **Stay on npm and pass `--ignore-scripts` everywhere (rejected).** It is
  ...
  straight back off; and it is a per-call-site flag — the moment one workflow,
   one CI step or one developer omits it, the vector is open again with
...
- **Bun (rejected here).** Attractive on install speed, but it is a runtime
  ...
  `engines` and the Vercel function runtime, and the
...
- **The `lock-lint` member of `check` changes shape.** Its subject is
  ...
  fails the same way in `check`, in CI and on Vercel — one rule across every
  packaged install path, instead of npm's "reinstall and see".
- **The npm-10-vs-11 pin story is obsolete.** `CLAUDE.md`'s toolchain
...
- **Corepack becomes load-bearing** for activating the pinned pnpm. It ships with
  ...
  pinned pnpm explicitly in CI — the `packageManager` field stays the
  single source of the version either way.
- **Every documented command example changes** (`npm run check` → the pnpm
  equivalent) across the README, `docs/`, `CLAUDE.md` and
...
- **Phantom-dependency breakage surfaces at migration time, not later.** Any
  ...
  `@opentelemetry/sdk-trace-base`, which npm resolved only because it hoisted the
  package out of `@opentelemetry/sdk-trace-node`; the fix was to declare it as
  the production dependency it always was.
