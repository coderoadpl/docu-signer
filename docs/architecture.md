...
shared sessions admit tenant peers and keep participant presence plus incoming
signatures in child rows without consuming the peer's own host-session slot,
then poll the tenant-scoped pad API without rendering archive navigation.
...
`document:read`, `document:write`, `document:approve`, `api-token:manage`,
`saved-search:manage` and `user-preference:manage`, and a pure
`decide(identity, capability)` predicate over
...
| `document_comments.body` is trimmed and 1–2000 characters | **DB + app** | comment policy → DB `CHECK` (`document_comments_body_check`) and the domain create schema. Core tests cover trimming and limits; repository integration rejects invalid raw values. |
