...
The approval is the one step only the owner can perform, from a device the
agent does not control; once it is given and the four checks are green, the
merge itself may be executed by the agent (`gh pr merge --merge`) or the
owner (both 2026-08-15 rulings archived in owner-rulings.md). The ruleset
dismisses stale approvals on every new push, so what merges is exactly the
snapshot the owner approved.
...
Decided by the **owner**, from a device the agent does not control. Opening
the PR and executing the merge may be delegated to an agent; **the approval
may not.**
...
never authorize one. Agents (acting as `chomamateusz-agent`) branch, open
PRs, dispatch workflows, and drive preview deployments freely — that is the
whole development environment. An agent may execute the merge
(`gh pr merge --merge`) **only after the owner's approving review** stands
next to the four green checks (the `require-gates` ruleset enforces both,
with no bypass); it cannot approve a PR (no self-approval), cannot lift the
rulesets past the owner's admin controls, and holds no platform-CLI session.
The approval — the release decision — is the owner's, from a device the
agent does not control, always (2026-08-15 rulings in owner-rulings.md).
